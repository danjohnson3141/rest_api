class UpdateGroupTypes < ActiveRecord::Migration
  def change

    return if Rails.env == "test"

    group_type = GroupType.find_by_id(1).try(:first)
    if !group_type.nil?
    ...
    group_type = GroupType.find_by_id(2).try(:first)
    if !group_type.nil?
      group_type.is_group_visible = 1
    ...
    group_type = GroupType.find_by_id(3).try(:first)
    if !group_type.nil?
